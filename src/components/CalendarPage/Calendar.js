...
export const CALENDAR_QUERY = gql`
         query PortalSiteHousesQuery($id: ID!, $house_id: ID!, $starts_at: Date!, $ends_at: Date!) {
           PortalSite(id: $id) {
             houses(id: $house_id) {
               id
               name
    ...
    renderCells(availabilities) {
                                ...
                                for (let daz of dayz) {
                                    for (let i = 0; i < 7; i++) {
                                      ...
                                      const cloneDay = day;
                                      days.push(<div className={`col cell ${!dateFns.isSameMonth(day, monthStart) ? "disabled" : dateFns.isSameDay(day, selectedDate) ? "selected" : ""} ${daz.arrival ? "arrival" : ""}`} key={day} onClick={() => this.onDateClick(dateFns.parse(cloneDay))}>
